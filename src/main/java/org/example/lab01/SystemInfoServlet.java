...
public class SystemInfoServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        int physCores = resolvePhysicalCores(logicalCores);
        ...
        String uptimeStr = String.format("%d г. %d хв. %d с.", uptimeSec / 3600, (uptimeSec % 3600) / 60, uptimeSec % 60);
        ...
        out.println("    .section{background:rgba(255,255,255,0.04);backdrop-filter:blur(12px);");
        out.println("             border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:28px;");
        out.println("             box-shadow:0 8px 32px rgba(0,0,0,0.3);transition:transform .3s,box-shadow .3s;}");
        out.println("    .section:hover{transform:translateY(-4px);box-shadow:0 16px 48px rgba(0,0,0,0.4);}");
        out.println("    .section-title{display:flex;align-items:center;gap:10px;font-size:0.75rem;font-weight:700;");
        out.println("                   letter-spacing:.15em;text-transform:uppercase;color:rgba(148,163,184,0.7);margin-bottom:20px;}");
        ...
        out.println("    .badge{display:inline-block;padding:3px 10px;border-radius:50px;font-size:0.8rem;font-weight:600;}");
        out.println("    .badge-blue{background:rgba(56,189,248,0.15);color:#38bdf8;border:1px solid rgba(56,189,248,0.3);}");
        out.println("    .badge-purple{background:rgba(167,139,250,0.15);color:#a78bfa;border:1px solid rgba(167,139,250,0.3);}");
        out.println("    .badge-green{background:rgba(52,211,153,0.15);color:#34d399;border:1px solid rgba(52,211,153,0.3);}");
        out.println("    .bar-wrap{width:100%;background:rgba(255,255,255,0.08);border-radius:6px;height:6px;margin-top:6px;}");
        out.println("    .bar{height:6px;border-radius:6px;background:linear-gradient(90deg,#38bdf8,#818cf8);}");
        out.println("    .center{text-align:center;}");
        out.println("    .back-link{display:inline-block;margin:32px auto 0;padding:12px 28px;");
        out.println("               background:linear-gradient(135deg,#38bdf833,#818cf833);");
        out.println("               border:1px solid rgba(56,189,248,0.4);border-radius:50px;color:#93c5fd;");
        out.println("               text-decoration:none;font-size:0.9rem;transition:all .3s;}");
        out.println("    .back-link:hover{background:linear-gradient(135deg,#38bdf855,#818cf855);transform:translateY(-2px);}");
        out.println("  </style>");
        out.println("</head>");
        out.println("<body>");
        ...
        out.println("    <div class=\"section-title\"><span>&#x26A1;</span>Процесор (CPU)</div>");
        row(out, "Логічних ядер / потоків", "<span class=\"badge badge-blue\">" + logicalCores + " threads</span>");
        row(out, "Фізичних ядер", "<span class=\"badge badge-purple\">" + physCores + " cores</span>");
        row(out, "Архітектура", osArch);
        row(out, "Завантаженість CPU", cpuLoadStr);
        out.println("  </div>");

        out.println("  <div class=\"section\">");
        out.println("    <div class=\"section-title\"><span>&#x1F4BE;</span>Оперативна пам'ять (RAM)</div>");
        row(out, "Усього RAM", formatBytes(totalRamBytes));
        row(out, "Вільно RAM", formatBytes(freeRamBytes));
        row(out, "Використано RAM", formatBytes(usedRam) + " <span style=\"color:rgba(148,163,184,0.6);font-size:.8rem\">(" + ramPct + "%)</span>");
        out.println("    <div class=\"bar-wrap\"><div class=\"bar\" style=\"width:" + ramPct + "%\"></div></div>");
        out.println("  </div>");

        out.println("  <div class=\"section\">");
        out.println("    <div class=\"section-title\"><span>&#x1F4BB;</span>Операційна система</div>");
        row(out, "ОС", osName);
        ...
        out.println("  <div class=\"section\">");
        out.println("    <div class=\"section-title\"><span>&#x2615;</span>Java Virtual Machine</div>");
        ...
        out.println("  <div class=\"section\">");
        out.println("    <div class=\"section-title\"><span>&#x23F1;&#xFE0F;</span>Runtime</div>");
    ...
    private int resolvePhysicalCores(int fallback) {
        try {
            String osName = System.getProperty("os.name").toLowerCase();
            ProcessBuilder pb;
            if (osName.contains("win")) {
                pb = new ProcessBuilder("cmd", "/c", "wmic cpu get NumberOfCores /value");
            } else {
                pb = new ProcessBuilder("sh", "-c", "cat /sys/devices/system/cpu/cpu*/topology/core_id | sort -u | wc -l");
            }
